...
sortingButton.addEventListener('click', () => {
  let newString =
  `<div id="imTired" class="form-group-one">
          ...
          <button type="submit" id="sortingHat" class="d-inline-flex btn btn-warning w-20">Submit</button>
    </form>
    </div>`;
...
const studentCardElemBuilder = () => {
 const muggleInput = document.getElementById('muggle').value;
  const whichHouse = houses[Math.floor(Math.random()*houses.length)];
    let newString = `<div class="card w-25" id ="thisWorks">
